...
[Ce projet s'inscrit dans une problématique de Data Science appliquée au marché immobilier californien. À partir du dataset House Prices (Kaggle, 1 460 biens, 81 variables), nous analysons les facteurs qui influencent le prix de vente d'un bien immobilier et construisons un pipeline complet de Data Science combinant Machine Learning tabulaire et Deep Learning sur images.

L'analyse quantitative de ce jeu de données est indispensable car le marché immobilier est fortement hétérogène : les prix varient de manière complexe et non-linéaire selon des dizaines de critères (qualité de construction, surface, localisation, âge du bien). Seule une approche algorithmique permet de modéliser ces interactions et d'objectiver les facteurs déterminants du prix.]
...
[La variable cible principale est **SalePrice** (prix de vente en dollars) — il s'agit d'une tâche de **régression supervisée**.

Le projet combine deux types de données complémentaires : des données tabulaires structurées (caractéristiques physiques du bien) et des données non structurées (photographies réelles des maisons), constituant une approche **multimodale**.

Les livrables analytiques produits sont :

- Un modèle **Random Forest Regressor** atteignant R²=0.73 (explique 73% de la variance des prix)
- Un **CNN TensorFlow** classifiant les biens en 3 catégories de prix avec 52.5% d'accuracy
- Un **dashboard interactif Plotly Dash** incluant un simulateur de prix en temps réel
- Un **rapport complet Quarto** documentant l'ensemble de la démarche scientifique]
